<?php
return [


     //*****************DASHBOARD */

     'patroling' => 'Rondaan',
     'total_patrolling_done' => 'Jumlah Rondaan Selesai',
     'total_notice_generated' => 'Jumlah Notis Dikeluarkan',
     'total_supervision' => 'Jumlah Penyeliaan',
     'substation' => 'Pencawang',
     'total_substation_visited' => 'Jumlah Pencawang Dikunjungi',
     'total_substation_defects' => 'Jumlah Kerosakan Pencawang',
     'feeder_pillar' => 'Feeder Pillar',
     'total_feeder_pillar_visited' => 'Jumlah Feeder Pillar Dikunjungi',
     'total_feeder_pillar_defects' => 'Jumlah Kerosakan Feeder Pillar',
     'tiang' => 'Tiang',
     'total_tiang_visited' => 'Jumlah Tiang Dikunjungi',
     'total_tiang_defects' => 'Jumlah Kerosakan Tiang',
     'link_box' => 'Link Box',
     'total_link_box_visited' => 'Jumlah Link Box Dikunjungi',
     'total_link_box_defects' => 'Jumlah Kerosakan Link Box',
     'cable_bridge' => 'Cable Bridge',
     'total_cable_bridge_visited' => 'Jumlah Cable Bridge Dikunjungi',
     'total_cable_bridge_defects' => 'Jumlah Kerosakan Cable Bridge',
 
     //*************************end dashboard */
 
     /************************side-bar and dahsboard */
 
     'dashboard' => 'Dashboard',
     '3rd_party_digging' => '3rd Party Digging',
     'create' => 'Cipta',
     'index' => 'Indeks',
     'patrolling' => 'Rondaan',
     'map' => 'Peta',
     'sbum_approval_and_detail' => 'Kelulusan SBUM dan Perincian',
     'substation' => 'Pencawang',
     'feeder_pillar' => 'Feeder Pillar',
     'tiang_talian_vt_&_vr' => 'Tiang + Talian VT & VR',
     'link_box_pelbagai_voltan' => 'Link Box Pelbagai Voltan',
     'cable_bridge' => 'Cable Bridge',
 
     /**************************end side-bar and dashboard */
 
     'add_notice' => 'Tambah Notis',
     'edit' => 'Ubah',
     'notice' => 'Notis',
     'zone' => 'Zone',
     'ba' => 'BA',
     'notice_form' => 'Borang Notis',
     'digging' => 'Pengorekan',
     'supervision' => 'Penyeliaan',
     'wp_name' => 'Work Package',
     'team_name' => 'Nama Team',
     'survey_date' => 'Tarikh Survey',
     'patrolling_time' => 'Waktu Rondaan',
     'project_name' => 'Nama Projek',
     'km_actual' => 'Jumlah Sebenar (KM)',
     'company_name' => 'Nama Kontraktor',
     'office_phone_no' => 'No. Telefon Pejabat',
     'main_contractor' => 'Main Kontraktor',
     'developer_phone_no' => 'No. Telefon Pembangun',
     'contractort_company_name' => 'Nama Syarikat Kontraktor',
     'site_supervisor_name' => 'Nama Penyelia Tapak',
     'site_supervisor_phone_no' => 'No. Telefon Penyelia Tapak',
     'excavator_operator_name' => 'Nama Operator Jentera',
     'excavator_machinery_reg_no' => 'No. Pendaftaran Jentera',
     'dpt_diging' => 'Jabatan Pengorekan',
     'survey_status' => 'Status Survey',
     'before_image_1' => 'Gambar Sebelum 1',
     'before_image_2' => 'Gambar Sebelum 2',
     'before_image_3' => 'Gambar Sebelum 3',
     'during_image_1' => 'Gambar Semasa 1',
     'during_image_2' => 'Gambar Semasa 2',
     'during_image_3' => 'Gambar Semasa 3',
     'after_image_1' => 'Gambar Selepas 1',
     'after_image_2' => 'Gambar Selepas 2',
     'after_image_3' => 'Gambar Selepas 3',
     'rd_name' => 'Nama Jalan',
     'submit' => 'Hantar',
     'update' => 'Kemaskini',
     'show' => 'Pamerkan',
     'no_image_found' => 'Gambar Tidak Ditemui',
 
     // *********************Substation    */
 
     'substation' => 'Pencawang',
     'visit_date' => 'Tarikh Lawatan',
     'patrol_time' => 'Masa Rondaan',
     'voltage' => 'Voltan',
     'name' => 'Nama',
     'type' => 'Jenis',
     'gate' => 'Pintu Pagar',
     'locked' => 'Berkunci',
     'unlocked' => 'Tidak Berkunci',
     'damaged' => 'Rosak',
     'others' => 'Lain-lain',
     'long_grass' => 'Bersemak Rumput Panjang',
     'tree_branches_in_PE' => 'Dahan Pokok Memasuki Compound PE',
     'building_defects' => 'Bangunan Defect',
     'broken_roof' => 'Bumbung Pecah',
     'broken_gutter' => 'Gutter Rosak',
     'broken_PE_base' => 'Base PE Pecah',
     'cleaning_illegal_ads_banners' => 'Pembersihan Iklan Haram/Banner',
     'image_gate' => 'Gambar Pintu Pagar',
     'image_grass' => 'Gambar Bersemak Rumput Panjang',
     'image_tree_branches' => 'Gambar Dahan Pokok Memasuki Compound PE',
     'images_gate_after_lock' => 'Gambar Pintu Pagar Selepas Kunci',
     'image_building' => 'Gambar Bangunan',
     'image_advertise_poster' => 'Gambar Iklan Haram/Banner',
     'other_image' => 'Lain-lain Gambar',
     'coordinate' => 'Koordinat',
 
 
     /*******************End substation  */
 
 
     /********************Feeder Pillar */
 
     'feeder_pillar' => 'Feeder Pillar',
     'size' => 'Saiz',
     'vandalism' =>'Vandalism',
     'leaning' => 'Condong',
     'leaning_angle' => 'Sudut Kecondongan',
     'rusty' => 'Berkarat',
     'image_vandalism' => 'Gambar Vandalism',
     'image_leaning'=> 'Gambar Condong',
     'image_rust' => 'Gambar Berkarat',
     
     'fp_gaurd'=>'FP Gaurd',
     'paint_faded'=>'Paint Faded',
     'name_plate'=>'Name Plate',
     /********************END Feeder Pillar */
 
 
     'from' => 'Daripada',
     'to' => 'Kepada',
     'cover_is_not_closed' => 'Sampul Tidak Ditutup',
     'bushy' => 'Bersemak',
     'cover_image' => 'Gambar Muka Depan',
     'image_bushes' => 'Gambar Bersemak',
  
 
  
     'pipe_broken' => 'Paip Pecah',
     'collapsed' => 'Runtuh',
     'image_pipe' => 'Gambar Paip',
     'image_collapsed' => 'Gambar Runtuh',
 
 
      /******************Tiang */
 
      'tiang'=>'Tiang',
      'qr_savr'=>'QR SAVR',
      'info' =>'Info',
          'contractor'=>'Contractor',
          'po_tart_date'=>'PO Start Date',
          'po_end_date' => 'PO End Date',
          'name_of_substation' =>'Name of Substation',
          'Name_of_Feeder_Pillar' =>'Name of Feeder Pillar',
          'Review_Date'=>'Review Date',
          'Feeder_Name'=>'Feeder Name',
          'Street_Name'=>'Street Name',
          'Section'=>'Section',
          'Tiang_No'=>'Tiang No',
  
      'Asset_Register'=>'Daftar Aset',
          'Pole_Size_Bill'=>'Bil Saiz Tiang',
          'Pole_type_No'=>'Bil Jenis Tiang',
          'Spun'=>'Spun',
          'Concrete'=>'Konkrit',
          'Iron'=>'Besi',
          'Wood'=>'Kayu',
          'BARE_Span'=>'BARE (Span)',
          'PVC_Span'=>'PVC (Span)',
          'ABC_Span'=>'ABC (Span)',
  
      'kejanggalan'=>'Kejanggalan',
          'title'=>'Title',
          'defects'=>'Defects',
          'images'=>'Images',
  
          'pole'=>'Tiang',
              'cracked'=>'Reput/Retak',
              'leaning'=>'Condong',
              'no_dim_post_none'=>'No. Tiang Pudar / Tiada',
              'creepers'=>'Creepers',
              'current_leakage'=>'Current Leakage',
  
          'line_main_service'=>'Talian (Utama / Servis)',
              'joint'=>'Penggunaan IPC/Maruku Joint Bagi sambungan tengah',
              'need_rentis'=>'Perlu Rentis',
              'Does_Not_Comply_With_Ground_Clearance'=>'Tidak Patuh Ground Clearance',
          
          'Umbang'=>'Umbang',
              'Sagging_Breaking'=>'Kendur/Putus',
              'Creepers'=>'Ulan (Creepers)',
              'No_Stay_Insulator_Damaged'=>'Tiada Stay Insulator/Rosak',
              'Stay_Plate_Base_Stay_Blocked'=>'Stay Plate / Pangkal Stay Terbongkah',
  
          'IPC'=>'IPC',
              'Burn Effect'=>'Kesan Bakar',
  
          'Black_Box'=>'Black Box',
              'Kesan_Bakar'=>'Kesan Bakar',
  
          'jumper'=>'Jumper',
              'no_uv_sleeve'=>'Tiada UV Sleeve',
  
          'lightning_catcher'=>'Penangkap Kilat',
              'broken'=>'Rosak',
  
          'Service'=>'Sesvis',
              'the_service_line_is_on_the_roof'=>'Talian servis berada di atas bumbung',
              'won_piece_date'=>'Won piece Tanggal',
  
          'grounding'=>'Pembumian',
              'no_connection_to_neutral'=>'Tiada Sambungan ke Neutral',
          
          'signage_off_point_two_way_supply'=>'Papan Tanda - OFF Point / Bekalan Dua Hala',
              'faded_damaged_missing_signage'=>'Papan Tanda Pudar / Rosak / Tiada',
              'main_street'=>'Sesalur Kaki Lima',
              'date_wire'=>'Wayar Tanggal',
              'junction_box_date_burn_effect'=>'Junction Box Tanggal / Kesan Bakar',
  
      
      'total_defects'=>'Bil Janggal',
      'planned_repair_date'=>'Tarikh Pembaikan Dirancang',
      'date_of_repair_performed'=>'Tarikh Pembaikan Dilaksanakan',
      'remarks'=>'Catitan',
  
      'Heigh_Clearance'=>'Height Clearance',
          'Site_Conditions'=>'Keadaan di Tapak',
          'Crossing_the_Road'=>'Melintasi Jalanraya',
          'Sidewalk'=>'Bahu Jalan',
          'No_vehicle_entry_area'=>'Kawasan tidak dimasuki kenderaan',
          'Area'=>'Kawasan',
          'Bend'=>'Bendang',
          'Road'=>'Jalanraya',
          'Forest'=>'Hutan',
          'Clearance_Distance'=>'Jarak Kelegaan (meter)',
          'Line_clearance_specifications'=>'Spesifikasi Kelegaan Talian',
          'Comply'=>'Patuh',
          'Disobedient'=>'Tidak Patuh',
  
      'Kebocoran_Arus'=>'Kebocoran Arus',
          'Inspection_of_current_leakage_on_the_pole'=>'Pemeriksaan Kebocoran Arus pada Tiang',
              'no'=>'Tidak',
              'yes'=>'Ya',
              'Amp'=>'(Amp)',
  
  
      /*******************ENd tiang */
  
  
      'are_you_sure' =>'Adakah Anda Pasti?',
      'detail'=>'Maklumat',
      'remove'=>'Buang',
      'request_failed'=>'Permohonan Gagal',
      'action'=>'Tindakan',
      'profile'=>'Profil',
      'login'=>' Log Masuk',
      'logout'=>'Log Keluar',
 
 
      /******************patroling */
 
 
     'Patrolling'=>'Rondaan',
         'Work_Package_Name'=>'Work Package',
         'Select_Road'=>'Pilih Jalan',
         'Road_Name'=>'Nama Jalan',
         'Patrolling_Date'=>'Tarikh Rondaan',
         'Feeder'=>'Feeder Pillar',
         'Project_Name' =>'Nama Projek',
         'Km_Plan'=>"Plan KM",
         'Km-Actual'=>'Actual KM',
         'Total_Digging'=>'Jumlah Pengorekan',
         'Total_Notice'=>'Jumlah Notis',
         'Total_Supervision'=>'Jumlah Penyeliaan',
 
 
 
      /**************End Patroling */

      'qr'=>'QR',
      'add_clean_up'=>'Add clean up',
      'image_advertise_poster_removal' =>'Advertisement Removal Image',
      'substation_feeder_pillar_name'=>'Substation / Feeder Pillar name',
      'fl_substation' =>'FL Substation',
      'main_line_service_line'=>'Main Line (M) / Servis (S)'
      
  
    // //*****************DASHBOARD */

    // 'patroling' => 'Rondaan',
    // 'total_patrolling_done' => 'Jumlah Rondaan Selesai',
    // 'total_notice_generated' => 'Jumlah Notis Dikeluarkan',
    // 'total_supervision' => 'Jumlah Penyeliaan',
    // 'substation' => 'Pencawang',
    // 'total_substation_visited' => 'Jumlah Pencawang Dikunjungi',
    // 'total_substation_defects' => 'Jumlah Kerosakan Pencawang',
    // 'feeder_pillar' => 'Feeder Pillar',
    // 'total_feeder_pillar_visited' => 'Jumlah Feeder Pillar Dikunjungi',
    // 'total_feeder_pillar_defects' => 'Jumlah Kerosakan Feeder Pillar',
    // 'tiang' => 'Tiang',
    // 'total_tiang_visited' => 'Jumlah Tiang Dikunjungi',
    // 'total_tiang_defects' => 'Jumlah Kerosakan Tiang',
    // 'link_box' => 'Link Box',
    // 'total_link_box_visited' => 'Jumlah Link Box Dikunjungi',
    // 'total_link_box_defects' => 'Jumlah Kerosakan Link Box',
    // 'cable_bridge' => 'Cable Bridge',
    // 'total_cable_bridge_visited' => 'Jumlah Cable Bridge Dikunjungi',
    // 'total_cable_bridge_defects' => 'Jumlah Kerosakan Cable Bridge',

    // //*************************end dashboard */

    // /************************side-bar and dahsboard */

    // 'dashboard' => 'Dashboard',
    // '3rd_party_digging' => '3rd Party Digging',
    // 'create' => 'Cipta',
    // 'index' => 'Indeks',
    // 'patrolling' => 'Rondaan',
    // 'map' => 'Peta',
    // 'sbum_approval_and_detail' => 'Kelulusan SBUM dan Perincian',
    // 'substation' => 'Pencawang',
    // 'feeder_pillar' => 'Feeder Pillar',
    // 'tiang_talian_vt_&_vr' => 'Tiang + Talian VT & VR',
    // 'link_box_pelbagai_voltan' => 'Link Box Pelbagai Voltan',
    // 'cable_bridge' => 'Cable Bridge',

    // /**************************end side-bar and dashboard */

    // 'add_notice' => 'Tambah Notis',
    // 'edit' => 'Ubah',
    // 'notice' => 'Notis',
    // 'zone' => 'Zone',
    // 'ba' => 'BA',
    // 'notice_form' => 'Borang Notis',
    // 'digging' => 'Pengorekan',
    // 'supervision' => 'Penyeliaan',
    // 'wp_name' => 'Work Package',
    // 'team_name' => 'Nama Team',
    // 'survey_date' => 'Tarikh Survey',
    // 'patrolling_time' => 'Waktu Rondaan',
    // 'project_name' => 'Nama Projek',
    // 'km_actual' => 'Jumlah Sebenar (KM)',
    // 'company_name' => 'Nama Kontraktor',
    // 'office_phone_no' => 'No. Telefon Pejabat',
    // 'main_contractor' => 'Main Kontraktor',
    // 'developer_phone_no' => 'No. Telefon Pembangun',
    // 'contractort_company_name' => 'Nama Syarikat Kontraktor',
    // 'site_supervisor_name' => 'Nama Penyelia Tapak',
    // 'site_supervisor_phone_no' => 'No. Telefon Penyelia Tapak',
    // 'excavator_operator_name' => 'Nama Operator Jentera',
    // 'excavator_machinery_reg_no' => 'No. Pendaftaran Jentera',
    // 'dpt_diging' => 'Jabatan Pengorekan',
    // 'survey_status' => 'Status Survey',
    // 'before_image_1' => 'Gambar Sebelum 1',
    // 'before_image_2' => 'Gambar Sebelum 2',
    // 'before_image_3' => 'Gambar Sebelum 3',
    // 'during_image_1' => 'Gambar Semasa 1',
    // 'during_image_2' => 'Gambar Semasa 2',
    // 'during_image_3' => 'Gambar Semasa 3',
    // 'after_image_1' => 'Gambar Selepas 1',
    // 'after_image_2' => 'Gambar Selepas 2',
    // 'after_image_3' => 'Gambar Selepas 3',
    // 'rd_name' => 'Nama Jalan',
    // 'submit' => 'Hantar',
    // 'update' => 'Kemaskini',
    // 'show' => 'Pamerkan',
    // 'no_image_found' => 'Gambar Tidak Ditemui',

    // // *********************Substation    */

    // 'substation' => 'Pencawang',
    // 'visit_date' => 'Tarikh Lawatan',
    // 'patrol_time' => 'Masa Rondaan',
    // 'voltage' => 'Voltan',
    // 'name' => 'Nama',
    // 'type' => 'Jenis',
    // 'gate' => 'Pintu Pagar',
    // 'locked' => 'Berkunci',
    // 'unlocked' => 'Tidak Berkunci',
    // 'damaged' => 'Rosak',
    // 'others' => 'Lain-lain',
    // 'long_grass' => 'Rumput Panjang',
    // 'tree_branches_in_PE' => 'Dahan Pokok dalam Pencawang',
    // 'building_defects' => 'Kerosakan Bangunan',
    // 'broken_roof' => 'Bumbung Pecah',
    // 'broken_gutter' => 'Gutter Rosak',
    // 'broken_PE_base' => 'Base PE Pecah',
    // 'cleaning_illegal_ads_banners' => 'Membersihkan Iklan Haram/Banner',
    // 'image_gate' => 'Gambar Pintu Pagar',
    // 'image_grass' => 'Gambar Rumput Panjang',
    // 'image_tree_branches' => 'Gambar Dahan Pokok',
    // 'images_gate_after_lock' => 'Gambar Pintu Pagar Selepas Kunci',
    // 'image_building' => 'Gambar Bangunan',
    // 'image_advertise_poster' => 'Gambar Iklan',
    // 'other_image' => 'Lain-lain Gambar',
    // 'coordinate' => 'Koordinat',


    // /*******************End substation  */


    // /********************Feeder Pillar */

    // 'feeder_pillar' => 'Feeder Pillar',
    // 'size' => 'Saiz',
    // 'vandalism' =>'Vandalisme',
    // 'leaning' => 'Condong',
    // 'leaning_angle' => 'Sudut Kecondongan',
    // 'rusty' => 'Berkarat',
    // 'image_vandalism' => 'Gambar vandalisme',
    // 'image_leaning'=> 'Gambar Condong',
    // 'image_rust' => 'Gambar Berkarat',

    // /********************END Feeder Pillar */


    // 'from' => 'Daripada',
    // 'to' => 'Kepada',
    // 'cover_is_not_closed' => 'Sampul Tidak Ditutup',
    // 'bushy' => 'Bushy',
    // 'cover_image' => 'Gambar Muka Depan',
    // 'image_bushes' => 'Gambar Semak',
 

 
    // 'pipe_broken' => 'Paip Pecah',
    // 'collapsed' => 'Runtuh',
    // 'image_pipe' => 'Paip Imej',
    // 'image_collapsed' => 'Imej Diruntuhkan',


    //  /******************Tiang */

    //  'tiang'=>'Tiang',
    //  'qr_savr'=>'QR SAVR',
    //  'info' =>'Info',
    //      'contractor'=>'Contractor',
    //      'po_tart_date'=>'PO Start Date',
    //      'po_end_date' => 'PO End Date',
    //      'name_of_substation' =>'Name of Substation',
    //      'Name_of_Feeder_Pillar' =>'Name of Feeder Pillar',
    //      'Review_Date'=>'Review Date',
    //      'Feeder_Name'=>'Feeder Name',
    //      'Street_Name'=>'Street Name',
    //      'Section'=>'Section',
    //      'Tiang_No'=>'Tiang No',
 
    //  'Asset_Register'=>'Asset Register',
    //      'Pole_Size_Bill'=>'Pole Size Bill',
    //      'Pole_type_No'=>'Pole Type No',
    //      'Spun'=>'Spun',
    //      'Concrete'=>'Concrete',
    //      'Iron'=>'Iron',
    //      'Wood'=>'Wood',
    //      'BARE_Span'=>'BARE (Span)',
    //      'PVC_Span'=>'PVC (Span)',
    //      'ABC_Span'=>'ABC (Span)',
 
    //  'kejanggalan'=>'Kejanggalan',
    //      'title'=>'Title',
    //      'defects'=>'Defects',
    //      'images'=>'Images',
 
    //      'pole'=>'Pole',
    //          'cracked'=>'Cracked',
    //          'leaning'=>'Leaning',
    //          'no_dim_post_none'=>'No. Dim Post / None',
    //          'creepers'=>'Creepers',
 
    //      'line_main_service'=>'Line (Main / Service)',
    //          'joint'=>'Joint',
    //          'need_rentis'=>'Need Rentis',
    //          'Does_Not_Comply_With_Ground_Clearance'=>'Does Not Comply With Ground Clearance',
         
    //      'Umbang'=>'Umbang',
    //          'Sagging_Breaking'=>'Sagging/Breaking',
    //          'Creepers'=>'Creepers',
    //          'No_Stay_Insulator_Damaged'=>'No Stay Insulator/Damaged',
    //          'Stay_Plate_Base_Stay_Blocked'=>'Stay Plate / Base Stay Blocked',
 
    //      'IPC'=>'IPC',
    //          'Burn Effect'=>'Burn Effect',
 
    //      'Black_Box'=>'Black Box',
    //          'Kesan_Bakar'=>'Kesan Bakar',
 
    //      'jumper'=>'Jumper',
    //          'no_uv_sleeve'=>'No UV Sleeve',
 
    //      'lightning_catcher'=>'Lightning catcher',
    //          'broken'=>'Broken',
 
    //      'Service'=>'Service',
    //          'the_service_line_is_on_the_roof'=>'The service line is on the roof',
    //          'won_piece_date'=>'Won piece Date',
 
    //      'grounding'=>'Grounding',
    //          'no_connection_to_neutral'=>'No Connection to Neutral',
         
    //      'signage_off_point_two_way_supply'=>'Signage - OFF Point / Two Way Supply',
    //          'faded_damaged_missing_signage'=>'Faded / Damaged / Missing Signage',
    //          'main_street'=>'Main Street',
    //          'date_wire'=>'Date Wire',
    //          'junction_box_date_burn_effect'=>'Junction Box Date / Burn Effect',
 
     
    //  'total_defects'=>'Total Defects',
    //  'planned_repair_date'=>'Planned Repair Date',
    //  'date_of_repair_performed'=>'Date of Repair Performed',
    //  'remarks'=>'Remarks',
 
    //  'Heigh_Clearance'=>'Heigh Clearance',
    //      'Site_Conditions'=>'Site Conditions',
    //      'Crossing_the_Road'=>'Crossing the Road',
    //      'Sidewalk'=>'Sidewalk',
    //      'No_vehicle_entry_area'=>'No vehicle entry area',
    //      'Area'=>'Area',
    //      'Bend'=>'Bend',
    //      'Road'=>'Road',
    //      'Forest'=>'Forest',
    //      'Clearance_Distance'=>'Clearance Distance',
    //      'Line_clearance_specifications'=>'Line clearance specifications',
    //      'Comply'=>'Comply',
    //      'Disobedient'=>'Disobedient',
 
    //  'Kebocoran_Arus'=>'Kebocoran Arus',
    //      'Inspection_of_current_leakage_on_the_pole'=>'Inspection of current leakage on the pole',
    //          'no'=>'no',
    //          'yes'=>'yes',
    //          'Amp'=>'(Amp)',
 
 
    //  /*******************ENd tiang */
 
 
    //  'are_you_sure' =>'Adakah Anda Pasti?',
    //  'detail'=>'Maklumat',
	//  'remove'=>'Buang',
    //  'request_failed'=>'Permohonan Gagal',
    //  'action'=>'Tindakan',
    //  'profile'=>'Profil',
    //  'login'=>' Log Masuk',
    //  'logout'=>'Log Keluar',


    //  /******************patroling */


    // 'Patrolling'=>'Rondaan',
    //     'Work_Package_Name'=>'Work Package',
    //     'Select_Road'=>'Pilih Jalan',
    //     'Road_Name'=>'Nama Jalan',
    //     'Patrolling_Date'=>'Tarikh Rondaan',
    //     'Feeder'=>'Feeder Pillar',
    //     'Project_Name' =>'Nama Projek',
    //     'Km_Plan'=>"Plan KM",
    //     'Km-Actual'=>'Actual KM',
    //     'Total_Digging'=>'Jumlah Pengorekan',
    //     'Total_Notice'=>'Jumlah Notis',
    //     'Total_Supervision'=>'Jumlah Penyeliaan',



    //  /**************End Patroling */
 
 
];
?>

